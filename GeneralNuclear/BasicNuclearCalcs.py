...
def production_decay(halfLife, n, t, rate, src, vol=1, tt=0.0):
    ...
    Calculates the initial population of atoms post-irradiation. Accounts for
    production and decay during the irradiation period. It optionally can
    calculate the number of atoms after a cool down period. Care should
    be taken with the units used for the input parameters as the function can
    handle both experimental and simulated calculations that use different
    ...
    Units:      [atoms]=[\f$\frac{n}{cm^2 s}\f$][\f$\frac{1}{cm}\f$]
                        [\f$cm^3\f$][s] \n\n
    ...
    @param rate: <em> integer or float </em>  \n
        The reaction rate. The reaction rate is typically in rx per src per
        cm$^3$ if volume is specified
        (for simulated output for example) or cm$^-1$ ($Sigma$) if
        experimental data is used.  n\n
    ...
    assert n >= 0, "The initial number of atoms specified must be greater \
                  than or equal to zero."
    assert t >= 0, "The sample decay time specified must be greater than \
                    or equal to zero."
    assert rate >= 0, "The production rate specified must be greater than \
                       or equal to zero."
    assert src >= 0, "The src specified must be greater than or equal to \
                      zero."
    assert vol > 0, "The volume specified must be greater than zero."
    assert tt >= 0, "The sample decay time specified must be greater than or \
                     equal to zero."
...
def decay(halfLife, n, t, units='uCi'):
    """!
    @ingroup BasicNuclearCalcs
    Calculates the activity or numnber of atoms following a decay period. \
    Cannot account for ingrowth terms.
    ...
    assert n >= 0, "The initial number of atoms specified must be greater than \
                  or equal to zero."
    assert t >= 0, "The sample decay time specified must be greater than or \
                  equal to zero."
    ...
    if units == "uCi":
        ...
        print "WARNING: Unknown activity units specified.  Valid specfications \
               are: 'uCi', 'Ci', or 'Bq', or 'atoms'. Decay calculations are \
               not to be trusted."
...
def get_halflife(decayConst):
    ...
    """
    assert decayConst > 0, "The decay constant specified must be greater than \
                          zero."
...
def activity(halfLife, n, t=0):
    """!
    @ingroup BasicNuclearCalcs
    Calculates the activity of a given isotope with n atoms at time t after
    production.  An initial activity can be specified instead of the initial
    number of atoms.
...
def solid_angle(a, d):
    ...
    assert a >= 0, "The detector radius specified must be greater than or \
                  equal to zero."
    assert d >= 0, "The distance to src specified must be greater than or \
                  equal to zero."
